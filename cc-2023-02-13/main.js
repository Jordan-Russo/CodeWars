...
function checkDigit(number, index1, index2, digit) {
  ...
  return number.toString().slice(minIndex, maxIndex + 1).includes(digit.toString());
};
// substring here isn't an option since they want it to include the highest index instead of stopping at it.
// For substring() start/end index doesn't matter but we need it to matter here since we need to increase the higher index by 1 to include the last term.